...
test('It responds to other HTTP methods', async () => {
    const res = await registry.handleRequest(new Request('https://example.com/posty/post', {
        method: 'POST',
...
test('It handles request payloads for application/json requests', async () => {
    const res = await registry.handleRequest(new Request('https://example.com/foo/1234/bar?baz=foo&baz=bar&baz=baz', {
        method: 'POST',
        body: JSON.stringify({
            foo: 'bar',
            foe: [
